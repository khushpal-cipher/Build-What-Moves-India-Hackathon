...
export default function PassportPathDesktop() {
  ...
  // UPDATED: Route guard checks all mandatory fields
  const isReadyToBook = (data: Partial<ApplicantData>) => {
    return !!(
      ...
      data.education &&
      data.employmentType &&
      data.distinguishingMark &&
      data.criminalRecord
    );
  };

  // UPDATED: Context-Aware Demo logic for 17 steps
  const getDemoAnswer = () => {
    if (!applicant.fullName) return "Aarav Kulkarni";
    if (!applicant.aliasName) return "None";
    if (!applicant.gender) return "Male";
    if (!applicant.dob) return "1998-05-14";
    if (!applicant.placeOfBirth) return "Pune, Maharashtra, India";
    if (!applicant.fatherName) return "Rajesh Kulkarni";
    if (!applicant.motherName) return "Smita Kulkarni";
    if (!applicant.spouseName) return "Not Married";
    if (!applicant.mobile) return "9876543210";
    if (!applicant.email) return "aarav.k@example.com";
    if (!applicant.currentAddress) return "123 FC Road, Pune, 411004";
    if (!applicant.previousAddress) return "No";
    if (!applicant.permanentAddress) return "Same";
    if (!applicant.education) return "Bachelor of Engineering";
    if (!applicant.employmentType) return "Private";
    if (!applicant.distinguishingMark) return "None";
    if (!applicant.criminalRecord) return "No";
    return "";
  };
  ...
  const handleInstantFill = () => {
    setApplicant({
      ...
      currentAddress: "123 FC Road, Pune, 411004",
      previousAddress: "No",
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        if (data.isValid) {
          ...
          alert(
            `PassportGuide AI Alert: This document does not look valid. Please upload a clearer copy.`,
          );
        }
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...prev,
        { role: "ai", text: "⚠️ Error connecting to API." },
      ]);
    } finally {
  ...
  // CLEANED: An array to render the Live Draft elegantly without repeating 17 divs
  const draftFields = [
        ...
        <aside className="w-[280px] border-r border-gray-100 flex flex-col justify-between shrink-0 bg-white">
          <div>
            <div className="p-8 pb-6 flex items-center gap-3 border-b border-gray-50">
              ...
              <div>
                <h1 className="text-xl font-bold text-gov-navy tracking-tight leading-none">
                  Passport Seva
                </h1>
                <p className="text-[10px] text-gov-orange font-bold tracking-wider uppercase mt-1">
                  Service Excellence
                </p>
              </div>
            </div>
            <div className="mt-4 px-4 space-y-2">
              <button
                ...
                className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-left text-sm font-semibold transition-colors ${activeTab === "documents" ? "bg-gov-navy text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}
              >
              ...
              <button
                onClick={() =>
                  canProceedToApp
                    ...
                    : alert("Upload required documents first.")
                }
              ...
              <button
                onClick={() =>
                  isReadyToBook(applicant)
                    ...
                    : alert("Complete the application first.")
                }
                className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-left text-sm font-semibold transition-colors ${activeTab === "appointment" ? "bg-gov-navy text-white shadow-md" : "text-gray-500 hover:bg-gray-50"} ${!isReadyToBook(applicant) && "opacity-50 cursor-not-allowed"}`}
              >
                <span className="text-lg">📅</span> 3. Book Appointment
              </button>
              <button
                onClick={() => setActiveTab("track")}
                className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-left text-sm font-semibold transition-colors ${activeTab === "track" ? "bg-gov-navy text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}
              >
          ...
          <header className="h-[100px] border-b border-gray-100 px-10 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search Passport FAQ's, etc"
                  className="w-[400px] bg-[#F5F5F5] text-sm text-gray-600 rounded-full py-3 pl-12 pr-6 focus:outline-none focus:ring-2 focus:ring-gov-navy/10"
                />
              </div>
            </div>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`w-10 h-5 rounded-full transition-colors relative ${isDemoMode ? "bg-gov-orange" : "bg-gray-200"}`}
                >
                  <div
                    className={`w-3.5 h-3.5 bg-white rounded-full absolute top-0.5 transition-transform ${isDemoMode ? "translate-x-[22px]" : "translate-x-1"}`}
                  />
                </button>
                <span className="text-[10px] font-bold text-gov-navy uppercase">
                  Demo
                </span>
              </div>
          ...
          <main className="flex-1 overflow-y-auto p-12 bg-white">
            ...
            {activeTab === "documents" && (
              <div className="max-w-[1200px]">
                <h2 className="text-4xl font-extrabold text-gov-navy mb-3">
                  Pre-Flight Document Check
                </h2>
                <p className="text-xl text-gray-500 mb-8">
                  Upload your required proofs here. PassportGuide AI will scan
                  them to ensure they match perfectly before you apply.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                  <div className="bg-[#F8FAFC] p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col h-[280px]">
                    ...
                    <label
                      ...
                      className={`mt-auto border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center transition-all ${docs.address ? "bg-[#E6F4EA] border-[#137333] cursor-default" : isScanning.address ? "bg-blue-50 border-blue-300 cursor-wait" : "bg-white border-[#C8E0FF] hover:bg-blue-50 cursor-pointer"}`}
                    >
                      {docs.address ? (
                        <>
                  ...
                  <div className="bg-[#F8FAFC] p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col h-[280px]">
                    <div className="mb-4">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Birth Certificate, PAN Card, Class 10
                      </p>
                    </div>
                    <input
                      type="file"
                      ...
                      className="hidden"
                      accept=".pdf,.jpg,.png"
                      onChange={(e) => handleFileUpload(e, "dob")}
                    />
                    <label
                      htmlFor="upload-dob"
                      className={`mt-auto border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center transition-all ${docs.dob ? "bg-[#E6F4EA] border-[#137333] cursor-default" : isScanning.dob ? "bg-blue-50 border-blue-300 cursor-wait" : "bg-white border-[#C8E0FF] hover:bg-blue-50 cursor-pointer"}`}
                    >
                      {docs.dob ? (
                        <>
                          <span className="text-3xl mb-2">✅</span>
                  ...
                  <div className="bg-[#F8FAFC] p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col h-[280px]">
                    <div className="mb-4">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        10th Pass Certificate or Higher
                      </p>
                    </div>
                    <input
                      type="file"
                      ...
                      className="hidden"
                      accept=".pdf,.jpg,.png"
                      onChange={(e) => handleFileUpload(e, "ecr")}
                    />
                    <label
                      htmlFor="upload-ecr"
                      className={`mt-auto border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center transition-all ${docs.ecr ? "bg-[#E6F4EA] border-[#137333] cursor-default" : isScanning.ecr ? "bg-blue-50 border-blue-300 cursor-wait" : "bg-white border-[#C8E0FF] hover:bg-blue-50 cursor-pointer"}`}
                    >
                      {docs.ecr ? (
                        <>
                          <span className="text-3xl mb-2">✅</span>
            ...
            {activeTab === "application" && (
              <div className="max-w-[1200px]">
                <h2 className="text-4xl font-extrabold text-gov-navy mb-8">
                  Passport Type
                </h2>
                <div className="flex gap-12">
                  <div className="flex-1">
                    <div className="bg-[#FFF4EA] border border-orange-200 p-5 rounded-2xl mb-6 shadow-sm">
                      <h4 className="font-extrabold text-gov-navy text-sm uppercase tracking-wider mb-3">
                        💡 Why a conversational form?
                      </h4>
                      <div className="grid grid-cols-3 gap-4 text-xs text-gray-700">
                        <div>
                          <strong className="block text-gov-orange mb-1">
                            Zero Cognitive Overload
                          </strong>
                          No massive forms. The AI asks one simple human
                          question at a time.
                        </div>
                        <div>
                          <strong className="block text-gov-orange mb-1">
                          ...
                          Confused by a term like "Non-ECR"? Just ask the chat
                          what it means.
                        </div>
                        <div>
                          <strong className="block text-gov-orange mb-1">
                          ...
                          Validates answers instantly to prevent rejections at
                          the passport office.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#F8FAFC] p-8 rounded-2xl border border-gray-100 h-[500px] flex flex-col">
                      <div className="flex-1 overflow-y-auto mb-6 space-y-4 pr-2">
                        {chatHistory.map((msg, idx) => (
                          <div
                            key={idx}
                            className={`p-5 rounded-xl shadow-sm border border-gray-100 text-sm leading-relaxed ${msg.role === "ai" ? "bg-white text-gray-800" : "bg-[#FFF4EA] text-gov-navy ml-12 border-[#F49931]/30"}`}
                          >
                      ...
                      >
                        ...
                        <div className="flex gap-4 mt-4">
                          ...
                          <button
                            ...
                            className="flex-1 bg-gov-navy text-white font-bold rounded-xl text-sm py-3 hover:bg-gov-navy/90"
                          >
                  ...
                  <div className="w-[350px]">
                    <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] sticky top-0 max-h-[700px] flex flex-col">
                      <div className="flex justify-between items-center mb-6 shrink-0">
                        ...
                        {isReadyToBook(applicant) ? (
                          <span className="bg-[#E6F4EA] text-[#137333] text-xs font-bold px-3 py-1 rounded-full">
                            Ready
                          </span>
                        ) : (
                      ...
                      <div className="flex-1 overflow-y-auto space-y-4 divide-y divide-gray-50 pr-2">
                        {draftFields.map((field, idx) => (
                          <div key={idx} className="pt-3 first:pt-0">
                            ...
                            <p
                              className={`text-sm font-semibold mt-1 ${field.value ? "text-gray-800" : "text-gray-300"}`}
                            >
                              {field.value || "Waiting..."}
                            </p>
                          </div>
                      ...
                      <button
                        ...
                        className={`w-full py-4 mt-6 rounded-xl font-bold text-sm transition-all duration-300 shrink-0 ${isReadyToBook(applicant) ? "bg-gov-orange text-white shadow-lg" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                      >
              ...
              <div className="max-w-[900px]">
                ...
                {!isBooked ? (
                  <div className="bg-[#F8FAFC] p-10 rounded-2xl border border-gray-100 shadow-sm">
                    <div className="space-y-4 mb-10 pb-8 border-b border-gray-200">
                      <div className="grid grid-cols-3">
                        ...
                        <p className="text-sm font-bold text-gov-navy col-span-2">
                          IND-2026-8849201
                        </p>
                      </div>
                      <div className="grid grid-cols-3">
                    ...
                    <h3 className="text-lg font-bold text-gov-navy mb-4">
                      1. Select Available Date (Pune PSK)
                    </h3>
                    <div className="flex gap-4 mb-10">
                      {["Aug 28", "Aug 29", "Sept 02", "Sept 03"].map(
                        (date) => (
                          <button
                            key={date}
                            onClick={() => setSelectedDate(date)}
                            className={`px-6 py-3 rounded-xl font-bold text-sm transition-all border-2 ${selectedDate === date ? "border-gov-navy bg-gov-navy text-white shadow-md" : "border-gray-200 bg-white text-gray-600 hover:border-gray-300"}`}
                          >
                            {date}
                          </button>
                        ),
                    ...
                    <div className="space-y-3 mb-10">
                      {[
                        ...
                        <label
                          key={method}
                          className={`flex items-center gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${selectedPayment === method ? "border-gov-orange bg-[#FFF4EA]" : "border-gray-200 bg-white hover:bg-gray-50"}`}
                        >
                          <input
                            type="radio"
                            name="payment"
                            ...
                            className="w-5 h-5 text-gov-orange focus:ring-gov-orange"
                          />
                    ...
                    <button
                      disabled={
                        !selectedDate || !selectedPayment || isProcessing
                      }
                      onClick={() => {
                      ...
                      className={`w-full py-5 rounded-xl font-bold text-lg transition-all flex justify-center items-center gap-3 ${selectedDate && selectedPayment ? "bg-gov-navy text-white shadow-lg hover:bg-gov-navy/90" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
                    >
                      {isProcessing ? (
                        <span className="animate-spin text-2xl">⚙️</span>
                      ) : (
                        `Pay ₹1,500 & Book Appointment`
                      )}
                    </button>
                  </div>
                ) : (
                  <div className="bg-[#E6F4EA] border border-[#137333]/20 p-10 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm">
                    <div className="w-20 h-20 bg-[#137333] text-white rounded-full flex items-center justify-center text-4xl mb-6 shadow-md">
                      ✓
                    </div>
                    <h3 className="text-3xl font-extrabold text-[#137333] mb-2">
                      Appointment Confirmed!
                    </h3>
                    <p className="text-gray-700 text-lg mb-8">
                      Your slot at Pune PSK is booked for{" "}
                      <span className="font-bold">{selectedDate}</span>. An SMS
                      has been sent to {applicant.mobile}.
                    </p>
                    <button
                      onClick={() => setActiveTab("track")}
                      className="bg-white text-[#137333] border border-[#137333] font-bold rounded-xl px-10 py-4 hover:bg-green-50 transition-all shadow-sm"
                    >
            ...
            {activeTab === "track" && (
              <div className="max-w-[1000px]">
                ...
                <div className="bg-[#F8FAFC] p-10 rounded-2xl border border-gray-100 shadow-sm flex gap-12">
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-gov-navy mb-8">
                      Application Status
                    </h3>
                    <div className="relative border-l-2 border-gray-200 ml-4 space-y-10">
                      <div className="relative pl-8">
                        ...
                        <p className="text-sm text-gray-500">
                          Form filled and fee paid successfully.
                        </p>
                        <span className="text-xs font-bold text-[#137333] mt-1 block">
                          Completed - {selectedDate || "Today"}
                        </span>
                      </div>
                      <div className="relative pl-8">
                        <div className="absolute -left-[11px] top-1 w-5 h-5 bg-[#137333] rounded-full border-4 border-[#F8FAFC]"></div>
                        <h4 className="font-bold text-gray-800">
                          PSK Appointment & Biometrics
                        </h4>
                        <p className="text-sm text-gray-500">
                          Documents verified at Passport Seva Kendra.
                        </p>
                        <span className="text-xs font-bold text-[#137333] mt-1 block">
                          Completed
                        </span>
                      ...
                      <div className="relative pl-8">
                        ...
                        <p className="text-sm text-gray-500">
                          Pending clearance from local police station.
                        </p>
                        <span className="text-xs font-bold text-gov-orange mt-1 block">
                          Delayed - Pending for 32 days
                        </span>
                      </div>
                      <div className="relative pl-8">
                        ...
                        <h4 className="font-bold text-gray-400">
                          Passport Printing & Dispatch
                        </h4>
                        <p className="text-sm text-gray-400">
                          Awaiting police clearance to begin printing.
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="w-[350px]">
                    <div className="bg-[#FFF4EA] border border-orange-200 p-8 rounded-2xl shadow-sm sticky top-0">
                      <h3 className="text-lg font-extrabold text-gov-navy mb-2 flex items-center gap-2">
                        <span className="text-2xl">⚠️</span> Verification
                        Delayed
                      </h3>
                      <p className="text-sm text-gray-700 mb-6 leading-relaxed">
                        Your application has been stuck at the Police
                        Verification stage for over 30 days. According to the
                        Citizen's Charter, you are eligible to file an official
                        grievance.
                      </p>
                      <button
                        onClick={() =>
                          alert(
                            "Simulating: PassportGuide AI is drafting a CPGRAMS grievance letter with File Ref No. IND-2026-8849201...",
                          )
                        }
                        className="w-full bg-gov-orange text-white font-bold rounded-xl py-4 shadow-md hover:bg-orange-500 transition-all flex justify-center items-center gap-2"
                      >
                        <span>📝</span> Auto-Draft Grievance
                      </button>
                      <p className="text-[10px] text-gray-400 mt-4 text-center leading-tight">
                        PassportGuide AI will format this perfectly for the
                        CPGRAMS portal.
                      </p>
                    </div>
                  </div>
